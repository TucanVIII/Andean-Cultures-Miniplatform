...
const createNewSection = asyncHandler(async (req, res) => {
  ...
  if (newSection) {
    res
      .status(201)
      .json({ message: `Section: ${sectionObject.sectionTitle} was created` });
  } else {
...
const deleteSection = asyncHandler(async (req, res) => {
  ...
  res.json({
    message: `Section ${section.sectionTitle} was removed successfully`,
  });
});
...
const updateSectionProgress = asyncHandler(async (req, res) => {
  ...
  if (!sectionId || !userEmail) {
    return res
      .status(404)
      .json({ message: "Section Id and user email are required" });
  }
  ...
  const updatedSection = updatedUser.sections.find(
    (sec) => sec.sectionId.toString() === sectionId
  );

  if (
    updatedSection.theoryCompleted &&
    updatedSection.videoCompleted &&
    updatedSection.quiz?.status === "completed"
  ) {
    await User.updateOne(
      { email: userEmail, "sections.sectionId": sectionId },
      { $set: { "sections.$.progress": "completed" }}
    );
  }
